...
> 目标：一行体现传统数据库原理（存储/缓冲池/解析器/执行器），一行容纳 agent 时代的新东西（KV/Doc/JSON、NL→SQL、MCP 工具）。
> 本文档是 pp-db 的唯一设计台账。
...
| 参考 | 借鉴什么 | 不学什么 |
|---|---|---|
| **bustub**（CMU 15-445） | P0 缓冲池（LRU）、P3 执行器算子结构 | C++ 复杂度、完整并发（pp-os 单核协程用不到 2PL） |
...
| **LevelDB / RocksDB** | **KV 有序映射：SkipList（移植简化，比 B+树简单）**、get/put/delete API | LSM 合并（agent 小数据过重） |
...
**移植原则**：存储内核/RDB 主体用 pp-lang 手写（教学主线）；**索引（SkipList/B+树）参考现成实现移植简化，不从头发明**。

---

## 3. 总体架构：一个存储内核，三种数据视图
...
```
页提供者接口（pp 函数指针/分发表）：
  db_page_alloc() / db_page_read(n) / db_page_write(n, buf)
pp-os：固定内存区实现
宿主机：文件/内存实现
```
...
键值页：header + 键值槽（有序，供 SkipList/扫描）
```

v1 字段类型：`INT`（32 位）、`STR(n)`（定长 ≤ 32）
v2：`TEXT`（变长）、`JSON`（复用 json.pp）
...
```
- WHERE：`= != < > <= >=`、`AND OR`、字面量/列引用
- 执行器：seq_scan / filter / project / insert / update / delete

**KV**：`db put <k> <v>` / `db get <k>` / `db del <k>`（LevelDB API 风格，有序映射：SkipList 参考移植）

**Doc**：`db doc put <name> <json>` / `db doc get <name>`（JSON 文档，复用 json.pp 校验/序列化）

**基础设施**：页式堆表 + 缓冲池（LRU ≤16 页）+ 二进制 `db save/load`
**集成**：`sql`/`db` 注册为 app；`db create/drop/list`

**验收**：
- [ ] RDB CRUD + WHERE/LIMIT 端到端可跑
- [ ] KV put/get/del 生效
- [ ] Doc put/get 生效（JSON 文档）
- [ ] `db save` 后 `db load` 数据一致（重启不丢）
- [ ] 宿主机宿主跑通（pp 编译器运行 + 持久化文件）
...
- **索引**：关系索引（`CREATE INDEX`）——参考 SQLite btree 思想移植简化（v2 末）
- **JSON 输出**：`SELECT ... TO JSON`（结果直接喂 LLM）

### v3 — 事务 + 并发（可选）

- `BEGIN / COMMIT / ROLLBACK`：UNDO 日志（SimpleDB 式）
- 表级锁 + 自旋锁（协程并发安全）
...
- [x] **D-1/D-2 独立分发**：文件抽象层（hf_* 双宿主）+ 宿主机 CLI（cli.pp）——验证通过（SQL 最小子集 CREATE/INSERT/SELECT WHERE、kv/doc 命令、save/load 真实文件往返；`pp obj cli.pp && cc` 产出 `ppdb` 可执行文件）
- [x] **D-3 宿主机测试脚本**：`tests/run_tests.sh`——验证通过（golden 测试 cases.txt vs expected.txt + 跨进程 save/load 往返；obj+cc 与 JIT 双路径）
- [ ] **P15-1** `db ask`（NL→操作）+ agent 数据模型（messages/kv/doc）
...
- [ ] **P15-3** 关系索引（CREATE INDEX，参考移植）+ JSON 输出
- [ ] **P16-1** 事务（BEGIN/COMMIT/ROLLBACK）+ 表锁
- [ ] **P16-2** `tools/ppdb-ref`（Rust 对照 + golden tests）
- [ ] **P16-3** 教程章节（数据库原理 × pp-lang × Agent）
