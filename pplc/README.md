...
`pp` 0.3.0 编译器：`.pp` 源码 → LLVM IR → 可执行 / JIT / 目标文件 / 裸机目标。

- 实现：**Rust + inkwell**（LLVM 18）
- 管线：`lexer → parser → AST → 泛型单态化 → sema → codegen(IR)`
...
- 类型：`int`（i32）、`float`（f64）、`bool`、`str {ptr,len}`、`u8/u16/u32/u64`、`void`、
  `struct`、Sum Type `enum`、显式泛型、数组 `[N]T`、指针 `*T`、函数指针、受限 tuple `(T1,T2)`
...
- 内置：`print` / `println`（int/float/bool/str，单实参）；系统层 `volatile_store8/16/32`、
  `volatile_load8/16/32`、`outb`/`inb`、`cli`/`sti`/`hlt`、`int_to_ptr`/`ptr_to_int`、`sizeof[T]`/`alignof[T]`

`pp run`（JIT）自动用 dlsym 解析 extern 函数（如 `printf`/`puts`）。extern 可以接收 `str`，
但不能返回缺少长度的 `str`；外部缓冲使用 `*u8`。

尚未支持：ARM64 裸机目标、复杂模式匹配、`pp test` 子命令等（见
