...
`pp` 编译器：`.pp` 源码 → LLVM IR → 可执行 / JIT / （后续）裸机 ELF。

- 实现：**Rust + inkwell**（LLVM 18）
- 管线：`lexer → parser → AST → codegen(IR)`
...
- 类型：`int`（i32）、`float`（f64）、`bool`、`str`、`u8/u16/u32/u64`、`void`、
  `struct`、数组 `[T; N]`、指针 `*T`
- 表达式：整数/浮点/字符串字面量、变量、二元运算、一元 `- !`、函数调用、
  结构体构造与字段访问、数组下标 `buf[i]`、指针 `&x`/`*p`/`p[i]`/`p+i`
- 语句：`let`（类型推断 + 零初始化）、赋值（含 `*p = v`、`buf[i] = v`）、
  `return`、表达式语句、`if/else`、`while`
...
`pp run`（JIT）自动用 dlsym 解析 extern 函数（如 `printf`/`puts`）。

尚未支持：`unsafe/asm/pointer`（系统层）、数组、`pp obj`/`pp test` 子命令等（见
